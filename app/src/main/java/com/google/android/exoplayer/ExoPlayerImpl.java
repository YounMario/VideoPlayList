...
/* package */ final class ExoPlayerImpl implements ExoPlayer {
  ...
  public ExoPlayerImpl(int rendererCount, int minBufferMs, int minRebufferMs, int startMs, int endMs) {
    ...
    internalPlayer = new ExoPlayerImplInternal(eventHandler, playWhenReady, selectedTrackIndices,
        minBufferMs, minRebufferMs, startMs, endMs);
  }
  ...
  public void setEndTime(int endTimeMs) {
    internalPlayer.setEndTime(endTimeMs);
  }
  ...
  /* package */ void handleEvent(Message msg) {
    switch (msg.what) {
      ...
      case ExoPlayerImplInternal.MSG_NOTIFY: {
        for (Listener listener : listeners) {
          listener.onNotify(DemoPlayer.CUSTOM_STATE_PAUSE);
        }
        break;
      }
      case ExoPlayerImplInternal.MSG_NOTIFY_END: {
        for (Listener listener : listeners) {
          listener.onNotify(DemoPlayer.CUSTOM_STATE_END);
        }
        break;
      }
    }
