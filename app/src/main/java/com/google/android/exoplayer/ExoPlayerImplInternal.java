...
/* package */ final class ExoPlayerImplInternal implements Handler.Callback {
  ...
  public static final int MSG_NOTIFY_END = 6;
  ...
  private long notifyTimeUs = 1000000;
  ...
  private void updatePositionUs() {
    ...
    if (notifyTimeUs > 0 && lastPos < notifyTimeUs && positionUs >= notifyTimeUs) {
      eventHandler.obtainMessage(MSG_NOTIFY).sendToTarget();
    }
    if (endTimeUs > 0 && lastPos < endTimeUs && positionUs >= endTimeUs) {
      eventHandler.obtainMessage(MSG_NOTIFY_END).sendToTarget();
    }
